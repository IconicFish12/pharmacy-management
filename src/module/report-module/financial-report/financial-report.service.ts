...
export class FinancialReportService {
  ...
  async getData(query: ReportQueryDto) {
    ...
    const totalExpenses = completedOrders.reduce(
  ...
  private generateCsvString(data: any): string {
    ...
    csvLines.push(
      `Total Revenue (Sales),IDR ${data.summary.totalRevenue.toLocaleString()}`,
    );
    csvLines.push(
      `Total Expenses (Stock Supply),IDR ${data.summary.totalExpenses.toLocaleString()}`,
    );
    csvLines.push(`Net Profit,IDR ${data.summary.netProfit.toLocaleString()}`);
    csvLines.push(
      `Total Customer Transactions,${data.summary.totalTransactionsCount}`,
    );
    csvLines.push(
      `Total Completed Restock Orders,${data.summary.totalOrdersCount}`,
    );
    csvLines.push('');
    ...
    const incomeHeaders = [
      ...
      'Cashier Name',
      'Total Price (IDR)',
      'Medicines Sold (Qty x UnitPrice)',
    ];
    ...
    for (const t of data.incomeBreakdown) {
      const itemsStr = t.transactionDetails
        .map(
          (d: any) =>
            `${d.medicine?.medicineName || 'Unknown'} (${d.quantity}x${d.unitPrice})`,
        )
        .join('; ');
    ...
    const expenseHeaders = [
      ...
      'Purchaser Name',
      'Total Price (IDR)',
      'Medicines Ordered (Qty x UnitPrice)',
    ];
  ...
  private async generateExcelBuffer(data: any): Promise<Buffer> {
    ...
    for (const t of data.incomeBreakdown) {
      const itemsStr = t.transactionDetails
        .map(
          (d: any) =>
            `${d.medicine?.medicineName || 'Unknown'} (${d.quantity}x${d.unitPrice.toLocaleString()})`,
        )
        .join(', ');
  ...
  private async generatePdfBuffer(
    ...
    return new Promise((resolve, reject) => {
      ...
      doc
        .fontSize(22)
        .font('Helvetica-Bold')
        .text('APOTHECARY FINANCIAL REPORT', 40, 25);
      doc
        .fontSize(10)
        .font('Helvetica')
        .text('Income Statement, Revenue & Expenses Breakdown', 40, 52);
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#2C3E50')
        .text('Financial Statement Summary', 40, doc.y);
      doc.y += 15;
      ...
      const metrics = [
        {
          ...
          value: `IDR ${data.summary.totalRevenue.toLocaleString()}`,
          color: '#27AE60',
        },
        {
          title: 'Total Expenses (Restock)',
          value: `IDR ${data.summary.totalExpenses.toLocaleString()}`,
          color: '#C0392B',
        },
        {
        ...
        },
        {
          ...
          value: `${data.summary.totalTransactionsCount} Orders`,
          color: '#2980B9',
        },
      ];
      ...
      metrics.forEach((m, idx) => {
        ...
        doc
          ...
          .fontSize(8)
          .font('Helvetica')
          .text(m.title, x + 8, doc.y + 12, { width: cardWidth - 16 });
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#2C3E50')
        .text('Sales & Revenue (Income)', 40, doc.y);
      doc.y += 15;
      ...
      data.incomeBreakdown.forEach((t: any) => {
        ...
        doc.text(t.employee?.name || 'System', 265, doc.y + 7, {
          width: 95,
          ellipsis: true,
        });
        ...
        doc
          .font('Helvetica-Bold')
          .text(t.totalPrice.toLocaleString(), 365, doc.y + 7);
        doc.font('Helvetica');

        const itemsStr = t.transactionDetails
          .map(
            (d: any) =>
              `${d.medicine?.medicineName || 'Obat'} (x${d.quantity})`,
          )
          .join(', ');
        doc.text(itemsStr, 445, doc.y + 7, {
          width: 105,
          height: rowHeight - 4,
          ellipsis: true,
        });
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#2C3E50')
        .text('Supply Restock Purchases (Expenses)', 40, doc.y);
      doc.y += 15;
      ...
      data.expenseBreakdown.forEach((o: any) => {
        ...
        doc.text(o.supplier?.companyName || 'N/A', 265, doc.y + 7, {
          width: 95,
          ellipsis: true,
        });
        ...
        doc
          .font('Helvetica-Bold')
          .text(o.totalPrice.toLocaleString(), 365, doc.y + 7);
        doc.font('Helvetica');
